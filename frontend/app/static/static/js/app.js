...
class PortalApp {
    ...
    async handleSendMessage() {
        ...
        try {
            ...
            if (data.response) {
                ...
                this.detectEmail(data.response); // auto fetch profile if email is mentioned by bot
            } else if (data.error) {
    ...
    async sendHiddenMessage(text) {
        try {
            ...
            if (data.response) {
                this.appendMessage(data.response, 'bot');
                this.detectEmail(data.response);
            }
        } catch (error) {
    ...
    async autoFetchProfile(email) {
        this.splitProfileContent.innerHTML = '<div style="text-align: center; padding: 2rem;">Fetching live profile data...</div>';
        try {
            const response = await fetch(`${this.apiBase}/api/candidate/${email}`);
            const data = await response.json();
